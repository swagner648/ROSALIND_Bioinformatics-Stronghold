...
file3 = [] # list of sequences
popls = [] # list of \n that need to be removed from the sequence
...
file = list(content) # seperates each element into it's own element of a list
for i in range(0, len(file)):
    if file[i] == "\n":
        popls.append(i) # marks all of the spots that a \n is present

for i in list(reversed(popls)): # removes all \n, needs to be flipped so that spaces aren't skipped
    file.pop(i) 
file2 = ''.join(file)   
file2 = file2.split('>')
file2.pop(0)
...
for i in file2:
    seqdict[i[13:]] = i[:13]  # creates a dictionary with  the sequence as the key and names as the value
